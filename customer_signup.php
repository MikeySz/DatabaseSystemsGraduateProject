...
if(empty(trim($clogin))) {
	die("Login is not valid! Empty! <br> <a href='customer_signup.html' target=_self>Return to Customer Signup</a>");
}
$clogin = trim($clogin, " ");

$cfname=mysqli_real_escape_string($con,$_POST["fname"]);
if(empty(trim($cfname))) {
	die("First Name is not valid! Empty! <br> <a href='customer_signup.html' target=_self>Return to Customer Signup</a>");
}
$cfname = trim($cfname, " ");

$clname=mysqli_real_escape_string($con,$_POST["lname"]);
if(empty(trim($clname))) {
	die("Last Name is not valid! Empty! <br> <a href='customer_signup.html' target=_self>Return to Customer Signup</a>");
}
$clname = trim($clname, " ");

$ctel=mysqli_real_escape_string($con,$_POST["tel"]);
if(empty(trim($ctel))) {
	die("Telephone is not valid! Empty! <br> <a href='customer_signup.html' target=_self>Return to Customer Signup</a>");
}
$ctel = trim($ctel, " ");

$caddress=mysqli_real_escape_string($con,$_POST["address"]);
if(empty(trim($caddress))) {
	die("Address is not valid! Empty! <br> <a href='customer_signup.html' target=_self>Return to Customer Signup</a>");
}
$caddress = trim($caddress, " ");

$ccity=mysqli_real_escape_string($con,$_POST["city"]);
if(empty(trim($ccity))) {
	die("City is not valid! Empty! <br> <a href='customer_signup.html' target=_self>Return to Customer Signup</a>");
}
$ccitye = trim($ccity, " ");

$czipcode=mysqli_real_escape_string($con,$_POST["zipcode"]);
if(empty(trim($czipcode))) {
	die("zipcode is not valid! Empty! <br> <a href='customer_signup.html' target=_self>Return to Customer Signup</a>");
}
$czipcode = trim($czipcode, " ");

$cstate=mysqli_real_escape_string($con,$_POST["state"]);
if(empty(trim($cstate))) {
	die("State is not valid! Empty! <br> <a href='customer_signup.html' target=_self>Return to Customer Signup</a>");
}
$cstate = trim($cstate, " ");
...
if(empty(trim($cpassword)) or empty(trim($cpassword2))) {
	die("Password is not valid! EMPTY Pasword detected! <br> <a href='customer_signup.html' target=_self>Return to Customer Signup</a>");
}
...
if($result2){
	...
	echo"<a href='CPS5740_p1.html' target=_self>Return to Project Phase 1 Page</a>";#return to Project 1 Phase1
}
echo"</body>";
echo "</HTML>";
mysqli_close($con);
